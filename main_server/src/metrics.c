...
void metricsSnapshot(const Metrics *m, MetricsSnapshot *out, int numDrivers, int surgeActive, float surgeMultiplier) {
	...
	pthread_mutex_t *mtx = (pthread_mutex_t *)&m->lock;
	...
	out->cancellationRate = (m->totalCreated > 0) ? (float)m->totalCancelled / (float)m->totalCreated : 0.0f;

	out->numRequestsNormal = m->countNormal;
	out->numRequestsVip = m->countVip;
	out->numRequestsEmergency = m->countEmergency;
	out->avgWaitNormal = (m->countNormal > 0) ? (float)(m->totalWaitNormal / (double)m->countNormal) : 0.0f;
	out->avgWaitVip = (m->countVip > 0) ? (float)(m->totalWaitVip / (double)m->countVip) : 0.0f;
	out->avgWaitEmergency = (m->countEmergency > 0) ? (float)(m->totalWaitEmergency / (double)m->countEmergency) : 0.0f;

	// Utilization: totalBusySeconds / (elapsed × numDrivers). Max possible value is exactly 1.0 (all drivers busy the whole time).
	double elapsed = difftime(time(NULL), m->startTime);
	if (elapsed > 0.0 && numDrivers > 0) {
		double util = m->totalBusySeconds / (elapsed * (double)numDrivers);
		if (util < 0.0) util = 0.0;
		if (util > 1.0) util = 1.0;
		out->driverUtilization = (float)util;
	}
...
void metricsReport(const Metrics *m, int numDrivers) {
	...
	printf("\n");
	printf("Requests by type (completed + cancelled):\n");
	printf("  Normal:        %d\n",   snap.numRequestsNormal);
	printf("  VIP:           %d\n",   snap.numRequestsVip);
	printf("  Emergency:     %d\n",   snap.numRequestsEmergency);
	printf("\n");
